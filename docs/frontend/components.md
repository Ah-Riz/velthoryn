...
Complete catalog of all 68 React components in `apps/web/src/components/`. Organized by category with props and usage examples.
...
File upload and preview panel for bulk CSV beneficiary upload. Handles RFC 4180 quoted fields, inline row validation errors, milestone support, and duplicate detection. Renders a preview table before submission.
...
| `onValidRows` | `(rows: PreparedRecipientRow[]) => void` | Callback with validated rows |
| `onError` | `(message: string) => void` | Callback on validation error |
| `mintDecimals` | `number` | Token decimals for amount precision validation |
...
CSV column format for linear (`releaseType=1`):
...
Dark modal for selecting the campaign mint. Sections: Popular Tokens and Wallet Tokens. SOL row shows "Wrap required" badge and opens `WrapSolModal`.
...
Modal for wrapping/unwrapping native SOL to wSOL (and back). Toggle between wrap/unwrap, amount input, balance display, Solscan link. Success state: green checkmark with auto-close after 2 seconds.
...
| `onSuccess` | `() => void` | Callback after successful wrap/unwrap |
...
| Prop | Type | Description |
|---|---|---|
| `initialRecipients` | `RecipientRow[]` | Current recipients with IDs |
| `loading` | `boolean` | Submission in progress |
| `onSubmit` | `(rows: RecipientRow[]) => Promise<void>` | Submit handler |
| `canRotate` | `boolean` | Whether rotation is allowed |
| `lockedReason` | `string \| null` | Reason rotation is disabled |
| `claimedAmounts` | `Record<string, number>` | Claimed per beneficiary |
| `mintDecimals` | `number` | Token decimals |

```tsx
import { AllocationEditor } from "@/components/campaign/detail/AllocationEditor";
...
<AllocationEditor
  ...
  claimedAmounts={Object.fromEntries(
    campaign.recipients.map((r) => [r.beneficiary, r.claimedAmount])
  )}
  mintDecimals={6}
/>
```
...
| Prop | Type | Description |
|---|---|---|
| `cancelledAtBigint` | `bigint \| null` | Cancellation timestamp |
| `isCreator` | `boolean` | Whether connected wallet is creator |
| `isInstantRefunded` | `boolean` | Instant refund state |
| `isFunded` | `boolean` | Whether campaign is funded |
| `nowTs` | `bigint` | Current timestamp |
| `onWithdrawClick` | `() => void` | Trigger withdraw flow |
| `unvestedAmount` | `bigint` | Remaining unvested tokens |
| `mintDecimals` | `number` | Token decimals |
...
Allows the beneficiary to close their on-chain `ClaimRecord` PDA after full claim or post-grace-period, recovering ~0.002 SOL rent.

**Pre-checks before building the instruction:**
1. Fetches `claimRecord` and `vestingTree` account infos in parallel.
2. If `claimRecord` is missing or not owned by the program, shows "already closed" toast.
3. If `vestingTree` is missing, shows a native-SOL-specific error (known SC-level limitation for fully-claimed native SOL campaigns).
4. Simulation errors routed through `formatVestingError`.
...
Empty state illustration and CTA for the campaigns list when no campaigns match the current filter.
...
Campaign lifecycle status badge. Maps `CampaignLifecycle` to a colored badge:
...
| `SmoothScrollLink` | Anchor link that scrolls smoothly |
