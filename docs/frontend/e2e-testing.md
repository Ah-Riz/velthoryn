...
Playwright E2E testing guide for the Velthoryn frontend. Covers setup, mock wallet architecture, writing tests, and CI integration.
...
All chromium tests use a mock wallet that bypasses real transaction signing. The mock is enabled by two mechanisms:

1. Setting `NEXT_PUBLIC_E2E_MOCK_WALLET=true` (dev server starts with this automatically).
...
See [ADR-FE-002](../decisions/adr-fe-002-e2e-mock-wallet.md) for the design rationale.
...
### Signing Test Additional Variables
...
| `user-journey.spec.ts` | Full creator to beneficiary golden path |
...
### Signing Specs (10 files)

Located in `tests/e2e/signing/`. These run against devnet with real wallet signing and skip CI by default.
...
test.describe("Feature Name", () => {
  ...
  test("should do something", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "My Campaigns" })
    ).toBeVisible();
  });
});
```
...
The `tests/e2e/helpers.ts` module exports:
...
- Playwright chromium tests run on every PR (`.github/workflows/ci.yml`).
