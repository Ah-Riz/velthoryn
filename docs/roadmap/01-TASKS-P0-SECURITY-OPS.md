...
- [ ] **Verify:** Unit test creates 11 requests to a 10/min endpoint; 11th returns `success: false`

### P0.2 — Wallet signature auth middleware
- [ ] Install `tweetnacl` and `@solana/web3.js` (already present)
- [ ] Create `apps/web/src/app/api/auth/nonce/route.ts`
  - `GET`: generate random nonce, store in Redis with 5min TTL, return `{ nonce, expiresAt }`
- [ ] Create `apps/web/src/lib/api/auth-middleware.ts`
...
- [ ] **Verify:** Unit test: valid signature passes; expired nonce fails; wrong signature fails; replayed nonce fails
...
- [ ] Create helper in `apps/web/src/lib/api/body-limit.ts`
...
- [ ] **Verify:** POST with 3MB body to `/api/campaigns` returns 413; 1MB body passes

### P0.5 — RLS verification + migration
- [ ] Create `apps/web/src/lib/db/migrations/0003_rls_policies.sql`
...
- [ ] Create `apps/web/tests/api/security.test.ts`
...
- [ ] All tests pass in CI (`pnpm test` in `apps/web/`)
...
- [ ] Create `apps/web/src/lib/api/logger.ts`
...
- [ ] **Verify:** Log output contains JSON with `requestId`, `method`, `path`, `timestamp` fields
...
- [ ] **Verify:** Throwing `new NotFoundError("Campaign")` returns `{ error: "Campaign not found", code: "NOT_FOUND", requestId: "..." }` with 404 status

### P1.3 — Health check endpoint
- [ ] Create `apps/web/src/app/api/health/route.ts`
  - `GET /api/health` (no auth, no rate limit)
  - Check DB: `db.execute(sql\`SELECT 1\`)` — timeout 5s
  - Check RPC: `connection.getHealth()` — timeout 5s
...
- [ ] **Verify:** `curl /api/health` returns 200 with `{ status: "ok", db: true, rpc: true }`

### P1.4 — DB connection pool tuning
- [ ] Update `apps/web/src/lib/db/index.ts`
...
- [ ] **Verify:** In production env, pool shows `max: 10` config
...
- [ ] Create `apps/web/src/lib/api/serialize.ts`
  - Export `serialize<T>(obj: T): Serialized<T>` that converts BigInt to string recursively
  - Apply to all API responses
- [ ] **Verify:** `JSON.stringify` on any API response never throws `TypeError: Do not know how to serialize a BigInt`

### P1.8 — Wrap existing routes with middleware
- [ ] Update all route handlers in `apps/web/src/app/api/`:
...
- [ ] **Verify:** All existing tests still pass after wrapping
...
- [ ] Create `apps/web/tests/api/error-handling.test.ts`
